...
| the design system package drifting from OpenDesign's authoring contract: an unparseable manifest, a missing USAGE.md section, or a component fixture referring to tokens it does not declare | `design-system-is-canonical.test.mjs`, over `design/backchannel/` |
...
checks that guard *reachability and contracts*; it is not the whole suite, which has 40 test files.
