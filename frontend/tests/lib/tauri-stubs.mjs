// The four things `DeviceSelection` reaches for, stubbed in one place so a test says what
// it depends on instead of hiding it in a config.
//
...
export function tauriStubs({ devices = [], onInvoke = () => {} } = {}) {
  const calls = [];
  const invoke = async (cmd, args) => {
    ...
    switch (cmd) {
      ...
      default: throw new Error(`DeviceSelection invoked an unstubbed command: ${cmd}`);
    }
  };
