...
export const AISummary = ({ summary, status, error, onSummaryChange }: Props) => {
  ...
  const renderErrorState = () => (
    <div className="w-full p-4 bg-danger-soft border border-danger/40 rounded-lg">
      <div className="flex items-center mb-2">
        <ExclamationTriangleIcon className="h-5 w-5 text-danger-ink mr-2" />
        <h3 className="text-danger-ink font-medium">Error Generating Summary</h3>
      </div>
      <p className="text-danger-ink text-sm">{error}</p>
      <p className="text-danger-ink text-xs mt-2">Please check your model configuration and API keys, or try again.</p>
    </div>
  );

  const renderLoadingState = () => (
    <div className="w-full p-4 bg-info-soft border border-info/40 rounded-lg">
      <div className="flex items-center space-x-3">
        <div className="animate-spin rounded-full h-5 w-5 border-2 border-info/40 border-t-transparent"></div>
        <div>
          <h3 className="text-info-ink font-medium">
            {status === 'processing' ? 'Processing Transcript' : 'Generating Summary'}
          </h3>
          <p className="text-info-ink text-sm">
            {status === 'processing' 
  ...
  if (!hasContent && status === 'completed') {
    return (
      <div className="w-full p-4 bg-sunken border border-line rounded-lg text-center">
        <p className="text-ink-muted">No summary content available.</p>
        <p className="text-ink-muted text-sm mt-1">Try generating a new summary.</p>
      </div>
    );
  }
  ...
  return (
    <div className="relative">
      ...
      {contextMenu.visible && selectedBlocks.length > 0 && (
        <div
          className="fixed z-dropdown bg-elevated shadow-lg rounded-lg py-1 min-w-[160px] border border-line
        ...
        >
          <button
            className="w-full px-4 py-2 text-left hover:bg-sunken flex items-center space-x-2"
          ...
          <button
            className="w-full px-4 py-2 text-left hover:bg-sunken text-danger-ink flex items-center space-x-2"
