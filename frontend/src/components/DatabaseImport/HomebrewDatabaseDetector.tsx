...
export function HomebrewDatabaseDetector({ onDecline }: HomebrewDatabaseDetectorProps) {
  ...
  return (
    <div className="mb-4 p-4 bg-info-soft border-2 border-info/40 rounded-lg">
      <div className="flex items-start gap-3">
        <Database className="h-6 w-6 text-info-ink mt-0.5 flex-shrink-0" />
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <AlertCircle className="h-4 w-4 text-info-ink" />
            <h3 className="text-sm font-semibold text-info-ink">
              Previous Meetily Installation Detected!
            </h3>
          </div>
          <p className="text-sm text-info-ink mb-2">
            We found an existing database from your previous Meetily installation (Python backend version).
          </p>
          <div className="bg-elevated/60 rounded p-2 mb-3">
            <p className="text-xs text-info-ink font-mono break-all">
              {detectedPath}
            </p>
          ...
          <p className="text-sm text-info-ink mb-3">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-brand text-brand-ink rounded-lg hover:bg-brand-hover disabled:bg-ink-faint disabled:cursor-not-allowed transition-colors"
            >
              {isImporting ? (
                <>
            ...
            <button
              onClick={handleNo}
              disabled={isImporting}
              className="flex-1 px-4 py-2 border-2 border-info/40 text-info-ink rounded-lg hover:bg-info-soft disabled:bg-sunken disabled:cursor-not-allowed transition-colors"
            >
              No, Browse Manually
            </button>
          </div>
