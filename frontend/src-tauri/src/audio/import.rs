...
async fn run_import<R: Runtime>(
    ...
    // One engine now, so there is nothing to branch on.
    let engine = if total_segments > 0 {
        Some(get_or_init_transcribe(&app, model.as_deref()).await?)
    } else {
        None
    };
    ...
    for (i, segment) in processable_segments.iter().enumerate() {
        ...
        let result = engine
            .as_ref()
            .expect("engine is initialized whenever there are segments")
...
///
/// `requested_model` overrides the stored config, which is how import offers
