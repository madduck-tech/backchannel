...
async fn run_retranscription<R: Runtime>(
    ...
    // One engine now, so there is nothing to branch on: ensure a model is
    // loaded and reuse it for every segment.
    let engine = get_or_init_transcribe(&app, model.as_deref()).await?;
...
/// Get or initialize the transcription engine, loading the configured model.
///
/// `requested_model` overrides the stored config, which is how the UI offers
/// "retranscribe with a different model".
async fn get_or_init_transcribe<R: Runtime>(
    app: &AppHandle<R>,
    requested_model: Option<&str>,
) -> Result<Arc<crate::transcribe_engine::TranscribeEngine>> {
    use crate::transcribe_engine::commands::{transcribe_init, TRANSCRIBE_ENGINE};

    transcribe_init()
        .await
        .map_err(|e| anyhow!("Failed to initialize transcription engine: {}", e))?;

    let engine = {
        let guard = TRANSCRIBE_ENGINE.lock().unwrap_or_else(|e| e.into_inner());
        guard.as_ref().cloned()
    }
    .ok_or_else(|| anyhow!("Transcription engine not initialized"))?;

    let target_model = match requested_model {
        Some(model) => model.to_string(),
        None => super::common::configured_local_model(app).await?,
    };

    if engine.get_current_model().await.as_deref() == Some(target_model.as_str())
        && engine.is_model_loaded().await
    {
        info!("Model '{}' already loaded", target_model);
        return Ok(engine);
    }

    // Populates the internal cache load_model checks against.
    if let Err(e) = engine.discover_models().await {
        warn!("Error during model discovery (continuing anyway): {}", e);
    }
